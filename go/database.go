...
func InitDB() (*sql.DB, error) {
	...
	dbOnce.Do(func() {
		...
		postgresErr := initPostgres()

		// If PostgreSQL fails, try SQLite
		if postgresErr != nil {
			log.Printf("PostgreSQL connection failed: %v. Trying SQLite...", postgresErr)
			sqliteErr := initSQLite()
			if sqliteErr != nil {
				err = fmt.Errorf("failed to connect to any database. PostgreSQL: %v, SQLite: %v", postgresErr, sqliteErr)
				return
			}
		}
...
func initPostgres() error {
	...
	// If any required PostgreSQL env vars are missing, return error
	...
	if dbErr != nil {
		return fmt.Errorf("failed to connect to PostgreSQL after multiple attempts: %v", dbErr)
	}
	...
	if _, err := db.Exec(initSchema); err != nil {
		return fmt.Errorf("failed to initialize PostgreSQL schema: %v", err)
	}
...
func initSQLite() error {
	...
	if dataDir == "" {
		// Default to ./data in development, /app/data in Docker
	...
	dbPath := fmt.Sprintf("%s/db.sqlite", dataDir)
	...
	CREATE TABLE IF NOT EXISTS ai_project_dbs (
		id INTEGER PRIMARY KEY AUTOINCREMENT,
	...
	-- Initial settings
	INSERT OR IGNORE INTO ai_settings (key, value, description, scope) 
	VALUES ('app_name', 'Data Manager', 'Application name', 'system');
	`
